    ...
    return stepsHeader?.map((v, i) => {
      return (
        <ComponentPanel key={i} {...v} value={stepIndex} index={i}>
          ...
          <TestComp inputValue={inputvalue} onTextChange={onTextChange} index={stepIndex} />
          {/* {v?.body?.component ? v.body.componentName + " Mapped" : v?.body} */}
        </ComponentPanel>
      );
    });
  };
            ...
            <Grid item md={3} xs={12}>
              {isControlleRequired && (
                <div style={{ marginTop: 20 , float:"right" }}>
                  ...
                  {activeStep === stepsHeader.length - 1 || activeStep === stepsHeader.length ?
                    <Button
                      className={classes.nextbutton}
                      variant="contained"
                      color={activeStep === stepsHeader.length - 1 ? "secondary" : "primary"}
